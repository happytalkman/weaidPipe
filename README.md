...
| `core/bots.py` | Bot Mode — 멀티에이전트 (역할·모델·메모리·봇간 대화) |
| `core/voice_profile.py` | 화자 인식 (자기상관 피치 분류·프로필 저장) |
| `core/hologram.py` | 3D 홀로그램 아바타 기하 코어 |
| `core/minutes.py` | 자동 회의록 생성 |
| `core/alarms.py` | 알람/타이머 (파싱·매니저) |
| `core/reporter.py` | 그래프 기반 보고서 자동 생성 |
...
| "봇 만들어줘 이름:OO 역할:OO" / "봇 OO에게 물어봐" | Bot Mode 생성/질문 |
| "봇 A랑 봇 B 대화시켜줘" | 봇↔봇 협업 대화 |
| "회의록 작성해줘" / "보고서 만들어줘" | 문서 자동 생성 |
| "10분 후에 알려줘" / "오후 3시 알람" | 알람/타이머 |
| "홀로그램 켜줘/꺼줘" | 3D 홀로그램 아바타 토글 |
| "내 이름은 OO야" | 화자 프로필 등록 |
